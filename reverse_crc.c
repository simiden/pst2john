...
void append_block(void *buffer, UInt32 size);

// Find reverse CRC patch
UInt8 *findReverse(UInt32 desiredCrc);

// Find reverse CRC patch
char *findReverseAscii(UInt32 desiredCrc);
...
//////////////////////////////////////////////////////////////////////////////
void CalculateTables(UInt32 polynomial)
{
	...
	for (int i = 0; i < 256; i++) {
		...
		for (int j = 0; j < 8; j++) {
			crc32Table[i] =
			    (crc32Table[i] << 1) ^ (crc32Table[i] & (1 << 31) ?
			    polynomial : 0);
		}
...
// 1 and 6, etc.
//////////////////////////////////////////////////////////////////////////////
UInt32 Reflect(UInt32 value, int count)
{
...
char *findReverseAscii(UInt32 desiredCrc)
{
	for (int a = 0; a < 62; a++) {
		for (int b = 0; b < 62; b++) {
			...
			append(TO_CHAR(a));
			append(TO_CHAR(b));
			UInt8 *reverse = findReverse(desiredCrc);
...
{
	char startStr[] = "abcde";
	//CalculateTables(0x04c11db7);
	append_block(startStr, strlen(startStr));
	//UInt32 crcOrig =   get();
	//UInt32 crcOrig = 0x2c60ac92;
	//UInt32 crcOrig =   0xc50309a7;
	UInt32 crcOrig =   0x50e099bc;
